...
import { Variant } from '../types'; // Adjusted path
...
type RouterLinkProps = Omit<ComponentProps<typeof Link>, 'to'> & { href: string }; // Custom type for react-router-dom Link
...
type Props = {
    ...
    className?: string;
    variant?: Variant;
} & (RouterLinkProps | ButtonProps); // Use RouterLinkProps here
...
const Button = ({
    ...
    if (as === 'link') {
        const props = rest as RouterLinkProps; // Cast to our custom type
        ...
        return (
            <Link className={buttonClasses} {...props} to={props.href || '#'}> {/* Changed href to to */}
                {variant !== 'link' && (
                    <span className="absolute top-[200%] left-0 right-0 h-full bg-white rounded-[50%] group-hover:top-0 transition-all duration-500 scale-150"></span>
                )}
                <span className="z-[1]">
